...
impl types::program::Program for ProgramSync {
    fn get_state(
    ...
    ) -> Result<types::state::State, String> {
        ...
        Ok(types::state::State {
            sources,
        })
    }

    fn make_site(
        &self, state: types::state::State,
    ) -> Result<types::site::Site, String> {
        println!("Making site");

        let empty_categories_map: HashMap<types::Title, Vec<types::state::Source>> = HashMap::new();

        let categories_map: HashMap<types::Title, Vec<types::state::Source>> =
            state.sources
                .iter()
                .fold(
                    empty_categories_map,
                    |mut acc, source| {
        ...
        let category_pages: Vec<types::site::CategoryPage> =
            result_ops::traverse_vec(
                categories_map
                    .iter()
                    .map(|(title, sources)| {
        ...
        let source_pages: Vec<types::site::SourcePage> = result_ops::traverse_vec(state.sources
            .iter()
            .map(|s| {
                types::site::Source::from_source_state(s.clone())
                    .render(&self.tera)
            })
            .collect()
        )?;
        ...
        let index_page: String =
            types::site::Index {
                categories: category_titles.iter()
